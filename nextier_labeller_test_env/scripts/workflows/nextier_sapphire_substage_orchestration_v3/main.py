"""Sapphire substage orchestration v3 -- same stores, new algorithm vintage.

Identical to v2 in every line that matters; what changed is the nextier-dash it
runs against (7303ffdc, the 2026-09-21 substage release) and therefore the
`algorithm_version` it stamps.

Why a new version string rather than re-running v2. The store keys already carry
it -- `sapphire_stage_uid` is "well:stage:algorithm_version" and
`sapphire_label_id` is "telemetry_point_id:algorithm_version" -- so v2 and v3 rows
COEXIST in the same three featurestores and neither overwrites the other. Re-running
v2 on the new code would instead upsert over the old rows under a label that no
longer describes them, and the fleet would end up half one vintage and half the
other with nothing to tell them apart.

What actually moves, measured on 212 stages over three ws1 wells with identical
inputs and only the package swapped:

    slurry_end      52 -> 59 placed, 51 of 52 kept placements move (median 30 s)
    ttr            113 -> 99  placed  -- 14 lost to "design rate never sustained after SS"
    rampdown_start  89 -> 77  placed  -- 10 of the 12 downstream of a lost ttr
    stage_end      169 -> 169 placed, 119 move (median 10 s)
    open_well / stage_start / pad_end / close_well: identical

The ttr loss is upstream acting deliberately: nextier-dash 797ec8ac restores the
CAUSAL design-rate chain (design_lookup -> the well's own memory -> whole-window
p95 as a batch-only last resort). The old path leaned on that p95, which reads the
stage's own future; the new one abstains rather than place a landmark off a
look-ahead reference. Correctness up, coverage down -- and since the anomaly gate
is (ttr, rampdown_start], fewer gated stages reach the anomaly pipeline.

Run v3 alongside v2 and compare before retiring v2. That is what the shared stores
and the versioned keys are for.
"""
...
async def nextier_sapphire_substage_orchestration_v3_flow(
    workspace_id: int,
